% You must load a new RECON2s model generated with the Jupyter Notebooks before running this script
...
biomass_rxn_id = findRxnIDs(model,'biomass_protein');
new = model;
for i=1:length(aas_ids)
    %new.S(aas_ids(i),biomass_rxn_id) = full(model.S(aas_ids(i),biomass_rxn_id))*0.8;
end

% The variable "new" now contains an RECON2s model with an adjusted biomass protein